...
foreach ($ips as $ip ) {
  ...
  if (!$result) {
    $msg = "FAILED: ".$checklist->message();
        $ip = getUserIP();
        tulis_file("../block_bot.txt","BLOCKED CUSTOM || user-agent : ".$_SERVER['HTTP_USER_AGENT']."\n ip : ". $ip." || ".gmdate ("Y-n-d")." ----> ".gmdate ("H:i:s")."\n\n");
        tulis_file("../result/total_bot.txt","$ip (CUSTOM BLOCKER)");
...
class IpBlockList {
  ...
  public function __construct( $whitelistfile = '../security/whitelist.dat',
                  $blacklistfile = '../security/user.dat' ) {
